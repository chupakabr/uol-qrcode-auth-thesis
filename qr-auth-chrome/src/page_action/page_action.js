...
chrome.extension.sendMessage({"name": "page_action.open"}, function(response) {
    ...
    (function authFilePolling() {
        $.ajax({
            url: "http://qrauth.chupakabr.ru/methods/get.php?id="+id,
            type: "GET",
            //data: {id: id},
            success: function(data, statusText, jqXHR) {
                ...
                console.warn("invalid data from the server");
            },
        }).done(function(data, statusText, jqXHR) {
            if (qrauth.waiting) {
                setTimeout(authFilePolling, 1000);
            }
        });
    })();
});
...
// move this kind of authorization steps to a server to be delivered on auth request
